...
if not exist "%SNAPSHOT_DIR%" (
    echo [ERROR] Chua co snapshot! Hay chay prepare_snapshot.bat truoc.
    pause
    exit /b 1
)
...
echo [4] Doi MachineGuid (may moi)...
...
echo     GUID moi: %NEW_GUID%

echo [5] Mo Evoto...
:: Doc path da luu (nhanh)
...
if defined EVOTO_EXE (
    ...
    echo     [WARN] Khong tim thay Evoto.exe. Hay mo thu cong.
)

echo DONE! Reset thanh cong + MachineGuid da doi.
exit
